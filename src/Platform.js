...
import { Sky, ContactShadows, RandomizedLight, AccumulativeShadows, softShadows } from "@react-three/drei";
    ...
    <group>
      {[0, 1, 2, 3, 4, 5].map((index) => (
        <BessContainer
          key={`bess-${index}`}
          position={[
            startX + index * containerSpacingX,
            4,
            startZ
          ]}
...
function SolarPanel({ scale = 1 }) {
  ...
  return (
    <group>
      {/* Panel frame */}
      <mesh>
        <boxGeometry args={[baseWidth * scale, baseHeight * scale, baseDepth * scale]} />
      ...
      <group position={[0, (baseHeight/2) * scale, 0]}>
        {/* Base surface */}
        <mesh>
          <boxGeometry args={[baseWidth * 0.97 * scale, baseHeight/2 * scale, baseDepth * 0.93 * scale]} />
          <meshPhongMaterial color="#1a237e" shininess={100} specular="#444444" />
        </mesh>
        ...
        {Array.from({ length: gridRows - 1 }).map((_, i) => (
          <mesh
            ...
            position={[
              ...
              (-baseDepth * 0.93 * scale * 0.5) + ((baseDepth * 0.93 * scale) / gridRows) * (i + 1)
            ]}
          >
            <boxGeometry args={[baseWidth * 0.97 * scale, 0.002, gridLineWidth * scale]} />
            {gridMaterial}
          </mesh>
        ))}
        ...
        {Array.from({ length: gridCols - 1 }).map((_, i) => (
          <mesh
            key={`v-${i}`}
            position={[
              (-baseWidth * 0.97 * scale * 0.5) + ((baseWidth * 0.97 * scale) / gridCols) * (i + 1),
              (baseHeight/2 * scale) + 0.001,
              0
            ]}
          >
            <boxGeometry args={[gridLineWidth * scale, 0.002, baseDepth * 0.93 * scale]} />
            {gridMaterial}
          </mesh>
        ))}
      </group>
...
export default function Platform() {
  ...
  return (
    <>
      ...
      <TransformerGroupSystem startX={transformerGroup_startX+10} z={transformerGroup_startZ} gap={20} />

      <Rack />
      <BessContainerGroup startX={-60} startZ={0} />

      <TransformerGroupSystem startX={transformerGroup_startX + 10} z={transformerGroup2_Z} gap={20} />
      {/* <Pillar /> */}
    </>
  );
}
